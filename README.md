...
*   **Login with default credentials**:
    *   Username: `admin`
    *   Password: `password`
*   **Register a new user**: On the login page, click "Need an account? Register" to create a new user with a default role of "Viewer".
...
**What it does:**
*   Automatically triggers on `push` to `main` branch and `pull_request` targeting `main`.
*   Builds and tests both the `api` and `dashboard` services.
*   Builds Docker images for both services and pushes them to GitHub Container Registry (`ghcr.io`), tagged with the commit SHA.
*   Ensures code quality and provides early feedback on changes.

**How to use it:**
Push your code to GitHub, or create a pull request. Monitor the status of the workflow runs in the "Actions" tab of your GitHub repository.
